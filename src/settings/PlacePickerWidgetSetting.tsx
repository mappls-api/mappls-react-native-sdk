...
const PlacePickerWidgetSetting: React.FC = () => {
  const instance = MapplsPlacePickerWidgetSettings.instance;
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const insets = useSafeAreaInsets();
  const [zoom, setZoom] = useState<string>(String(instance.zoom ?? ''));
  ...
  const [historyCount, setHistoryCount] = useState<string>(String(instance.historyCount ?? ''));
  ...
  const [location, setLocation] = useState(String(instance.location || ''));
  const [filter, setFilter] = useState<string>(instance.filter ?? '');
  const [deBounce, setDeBounce] = useState<string>(String(instance.deBounce ?? ''));
    ...
    <View style={{ backgroundColor: colors.backgroundPrimary, flex: 1, paddingTop: insets.top, paddingBottom: insets.bottom, paddingLeft: insets.left, paddingRight: insets.right }}>
         <View style={styles.header}>
                <View style={styles.leftSection}>
                    ...
                    <Text style={styles.headerTitle}>Place Picker Widget Setting</Text>
                </View>
            </View>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.inputGroup}>
          ...
          />
           <TouchableOpacity style={styles.button} onPress={() => {
           instance.historyCount = parseInt(historyCount);
          }}>
          ...
          <TouchableOpacity style={styles.button} onPress={() => {
            ...
            if (parts.length === 2) {
              ...
              if (!isNaN(lat) && !isNaN(lng)) {
                const loc: [number, number] = [lng, lat]; // [lng, lat] format
                instance.location = loc;     // keep instance updated
              }
            }
          }}>
        ...
        <View style={styles.inputGroup}>
          ...
          />
          <TouchableOpacity style={styles.button} onPress={() => {
            instance.deBounce = parseInt(deBounce);
          }}>
...
export default PlacePickerWidgetSetting;
