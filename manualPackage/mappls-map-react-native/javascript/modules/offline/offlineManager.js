...
class OfflineManager {
   ...
   *
   * await MapplsGL.offlineManager.createPack({
   *   name: 'offlinePack',
   *   styleURL: 'mapbox://...',
   *   minZoom: 14,
   *   maxZoom: 20,
  ...
  async createPack(options, progressListener, errorListener) {
    ...
    const nativeOfflinePack = await MapplsGLOfflineManager.createPack(
      packOptions,
    );
   ...
   *
   * @example
   * await MapplsGL.offlineManager.invalidatePack('packName')
   *
  ...
  async invalidatePack(name) {
    ...
    if (offlinePack) {
      await MapplsGLOfflineManager.invalidatePack(name);
    }
  }
   ...
   *
   * @example
   * await MapplsGL.offlineManager.deletePack('packName')
   *
   * @param  {String}  name  Name of the offline pack.
   * @return {void}
   */
  async deletePack(name) {
    ...
    const offlinePack = this._offlinePacks[name];
    if (offlinePack) {
      await MapplsGLOfflineManager.deletePack(name);
   ...
   *
   * @example
   * await MapplsGL.offlineManager.invalidateAmbientCache();
   *
   * @return {void}
   */
  async invalidateAmbientCache() {
    await this._initialize();
    await MapplsGLOfflineManager.invalidateAmbientCache();
  }
   ...
   *
   * @example
   * await MapplsGL.offlineManager.clearAmbientCache();
   *
   * @return {void}
   */
  async clearAmbientCache() {
    await this._initialize();
    await MapplsGLOfflineManager.clearAmbientCache();
  }
   ...
   *
   * @example
   * await MapplsGL.offlineManager.setMaximumAmbientCacheSize(5000000);
   *
  ...
  async setMaximumAmbientCacheSize(size) {
    await this._initialize();
    await MapplsGLOfflineManager.setMaximumAmbientCacheSize(size);
  }
   ...
   *
   * @example
   * await MapplsGL.offlineManager.resetDatabase();
   *
   * @return {void}
   */
  async resetDatabase() {
    await this._initialize();
    await MapplsGLOfflineManager.resetDatabase();
  }
   ...
   *
   * @example
   * const offlinePacks = await MapplsGL.offlineManager.getPacks();
   *
   ...
   *
   * @example
   * const offlinePack = await MapplsGL.offlineManager.getPack();
   *
   * @param  {String}  name  Name of the offline pack.
   * @return {OfflinePack}
   */
   ...
   *
   * @example
   * await MapplsGL.offlineManager.mergeOfflineRegions(path);
   *
  ...
  async mergeOfflineRegions(path) {
    await this._initialize();
    return MapplsGLOfflineManager.mergeOfflineRegions(path);
  }
   ...
   *
   * @example
   * MapplsGL.offlineManager.setTileCountLimit(1000);
   *
   * @param {Number} limit Map tile limit count.
   * @return {void}
   */
  setTileCountLimit(limit) {
    MapplsGLOfflineManager.setTileCountLimit(limit);
  }
   ...
   *
   * @example
   * MapplsGL.offlineManager.setProgressEventThrottle(500);
   *
   * @param {Number} throttleValue event throttle value in ms.
   * @return {void}
   */
  setProgressEventThrottle(throttleValue) {
    MapplsGLOfflineManager.setProgressEventThrottle(throttleValue);
  }
   ...
   * MapplsGL.offlineManager.subscribe('packName', progressListener, errorListener)
   *
  ...
  async subscribe(packName, progressListener, errorListener) {
    ...
    if (isFunction(progressListener)) {
      if (totalProgressListeners === 0) {
        this.subscriptionProgress = OfflineModuleEventEmitter.addListener(
          MapplsGL.OfflineCallbackName.Progress,
          this._onProgress,
        );
      }
    ...
    if (isFunction(errorListener)) {
      if (totalErrorListeners === 0) {
        this.subscriptionError = OfflineModuleEventEmitter.addListener(
          MapplsGL.OfflineCallbackName.Error,
          this._onError,
        );
      }
    ...
    if (isAndroid() && this._offlinePacks[packName]) {
      try {
        ...
        await MapplsGLOfflineManager.setPackObserver(packName);
      } catch (e) {
   ...
   *
   * @example
   * MapplsGL.offlineManager.unsubscribe('packName')
   *
   * @param  {String} packName Name of the offline pack.
   * @return {void}
   */
  ...
  async _initialize() {
    ...
    const nativeOfflinePacks = await MapplsGLOfflineManager.getPacks();
  ...
  _onProgress(e) {
    ...
    if (state === MapplsGL.OfflinePackDownloadState.Complete) {
